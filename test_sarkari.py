...
MAX_PAGES = 50 # Kitne pages tak scrape karna hai
...
# -------------------------------------------------------------
# Main Execution Pipeline (Parses Tables Directly From Listing Page)
# -------------------------------------------------------------
def run_indgovtjobs_table_scraper():
    ...
    for page_num in range(1, MAX_PAGES + 1):
        ...
        print(f"🌐 Category Page Table Scraping [{page_num}/{MAX_PAGES}]: {page_url}")

        try:
            res = requests.get(page_url, headers=HEADERS, timeout=15)
            if res.status_code != 200:
                print(f"❌ Failed to load page {page_num}, Status: {res.status_code}")
                break
            ...
            for table in tables:
                ...
                # If no direct title in key-value, try finding header link above table
                if not title:
    ...
    print(f"🎉 Complete! Directly extracted and saved {len(job_cards)} active jobs into '{JSON_FILENAME}'.")
