...
import cloudscraper  # 🛡️ Anti-Cloudflare Bypass Tool
...
AI_MODELS_TIERS = [
    "llama-3.1-8b-instant",     # Tier 1: Fast & Low Token
    "mixtral-8x7b-32768",       # Tier 2: Mid-range MoE
    "llama-3.3-70b-versatile"   # Tier 3: Final Fallback
]
...
# -------------------------------------------------------------
# 4. SECONDARY SOURCE: SarkariResult via CloudScraper (Bypasses 403)
# -------------------------------------------------------------
def fetch_sarkari_result_data_cloudscraper(url):
    candidates = []
    print(f"\n[SECONDARY SOURCE] 🛡️ Scraping SarkariResult via CloudScraper: {url}")
    
    try:
        # Create CloudScraper engine to bypass Cloudflare
        scraper = cloudscraper.create_scraper(browser={'browser': 'chrome', 'platform': 'windows', 'mobile': False})
        res = scraper.get(url, timeout=20)

        if res.status_code != 200:
            print(f"[SECONDARY SOURCE] ❌ Cloudscraper HTTP Status: {res.status_code}")
            return candidates

        print(f"[SECONDARY SOURCE] ✅ Cloudflare Successfully Bypassed! Status Code 200")
        ...
        print(f"[SECONDARY SOURCE] ✅ Extracted {len(candidates)} active links from SarkariResult!")
...
# 5. Inner Page Fetch
# -------------------------------------------------------------
def fetch_deep_details(detail_url, source="FreeJobAlert"):
    details = {"text_content": ""}
    try:
        if source == "SarkariResult":
            scraper = cloudscraper.create_scraper(browser={'browser': 'chrome', 'platform': 'windows', 'mobile': False})
        ...
        print(f"⚠️ Inner page warning ({detail_url}): {e}")
...
def run_job_pipeline():
    ...
    # Step A: Primary Source (FreeJobAlert)
    primary_url = "https://www.freejobalert.com/latest-notifications/"
    candidates = fetch_main_table_data(primary_url)

    # Step B: Secondary Fallback (SarkariResult via Cloudscraper) if Primary Fails
    if not candidates:
        print("\n⚠️ Primary source failed or empty. Triggering Secondary Source (SarkariResult via Cloudscraper)...")
        secondary_url = "https://www.sarkariresult.com/latestjob/"
        candidates = fetch_sarkari_result_data_cloudscraper(secondary_url)
    ...
    for idx, cand in enumerate(candidates[:25]):
        ...
        print(f"[{idx+1}/{len(candidates[:25])}] 🔗 [{cand.get('source')}] Processing: {title}")

        deep_data = fetch_deep_details(detail_url, cand.get("source"))
