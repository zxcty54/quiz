...
# Helper function to check if date is strictly yesterday
def is_strictly_yesterday(date_obj, source_name, title_str):
    if not date_obj:
        debug(f"{source_name} SKIPPED NO DATE | {title_str[:80]}")
        return False

    pub_date = date_obj.date() if isinstance(date_obj, datetime) else date_obj
    debug(f"{source_name} PUBLISHED DATE: {pub_date} (Target Yesterday: {YESTERDAY}) | {title_str[:80]}")

    if pub_date == YESTERDAY:
        return True
    else:
        debug(f"{source_name} SKIPPED OLD/INVALID DATE | {pub_date} | {title_str[:80]}")
        return False
...
def fetch_url(url):
    ...
    debug(f"Retrying with SSL verification disabled: {url}")
    ...
    debug(f"Retrying normal requests verify=False: {url}")
    html = normal_fetch(url, verify=False)
    if html:
        return html

    if SCRAPINGANT_API_KEY:
        debug(f"Trying ScrapingAnt: {url}")
...
def extract_article_content(soup, source=""):
    ...
    for tag in soup(["script", "style", "noscript", "svg", "canvas", "iframe", "nav", "footer", "form", "aside"]):
        tag.decompose()
    ...
    selectors = [
        ...
        ".press-release-content", ".pressRelease", ".release-content",
        ".content-area", ".main-content", ".post-content", ".entry-content",
        ".single-content", ".td-post-content", ".content", "main",
    ]
    ...
    for text in candidates:
        ...
        if not text or is_bad_portal_content(text):
            continue
        cleaned.append(text)
...
def fetch_generic_article_content(url, source=""):
    ...
    for candidate_url in candidate_urls:
        ...
        debug(
            ...
            f"{chars} chars | {words} words | {candidate_url}"
        )
...
def make_item(source, title, url, date=None, content="", item_type="Scraped"):
    ...
    if not valid:
        warn(
            ...
            f"{source} | {chars} chars | {words} words | {title[:100]}"
        )
        return None
...
def deduplicate(items):
    ...
    for item in items:
        ...
        key = hashlib.sha1(
            normalize_for_hash(key).encode("utf-8", errors="ignore")
        ).hexdigest()
...
def scrape_pib():
    ...
    for item in entries:
        ...
        if not is_strictly_yesterday(date, "PIB", title):
            continue

        content, article_date = fetch_generic_article_content(url, "PIB")
        if not content:
            continue

        final_date = article_date or date
        obj = make_item(
            source="PIB",
            title=title,
            url=url,
    ...
    if not results:
        debug("PIB: Kal (Yesterday) ka koi data nahi mila.")
...
def scrape_news_on_air():
    ...
    for title, url in unique:
        ...
        if not is_strictly_yesterday(date, "NEWS ON AIR", title):
            continue
    ...
    results = deduplicate(results)
    if not results:
        debug("NEWS ON AIR: Kal (Yesterday) ka koi data nahi mila.")
...
def scrape_cmo_bihar():
    ...
    for title, url in unique:
        ...
        if not is_strictly_yesterday(date, "CMO BIHAR", title):
            continue
    ...
    results = deduplicate(results)
    if not results:
        debug("CMO BIHAR: Kal (Yesterday) ka koi data nahi mila.")
...
def scrape_iprd_bihar():
    ...
    for title, url in unique:
        ...
        if not is_strictly_yesterday(date, "IPRD BIHAR", title):
            continue
    ...
    results = deduplicate(results)
    if not results:
        debug("IPRD BIHAR: Kal (Yesterday) ka koi data nahi mila.")
...
def scrape_bihar_cabinet():
    ...
    for page_url in CABINET_PAGES:
        print(f"🔎 Cabinet Listing: {page_url}")
        html = fetch_url(page_url)
        if not html:
            continue
    ...
    for title, url in unique:
        ...
        if not is_strictly_yesterday(date, "BIHAR CABINET", title):
            continue
    ...
    results = deduplicate(results)
    if not results:
        debug("BIHAR CABINET: Kal (Yesterday) ka koi data nahi mila.")
...
# ============================================================
# 6. SANSAD TV (STRICT YESTERDAY-ONLY)
# ============================================================
...
def scrape_sansad_tv():
    ...
    for page_url in SANSAD_PAGES:
        ...
        for a in soup.find_all("a", href=True):
            ...
            low = title.lower()
            if any(x in low for x in ["home", "login", "contact", "privacy", "terms"]):
                continue
    ...
    results = []
    for title, url in unique:
        content, date = fetch_generic_article_content(url, "Sansad TV")
        if not content:
            continue

        if not is_strictly_yesterday(date, "SANSAD TV", title):
            continue
    ...
    results = deduplicate(results)
    if not results:
        debug("SANSAD TV: Kal (Yesterday) ka koi data nahi mila.")

    print(f"✅ Sansad TV usable: {len(results)}")
    return results
...
def scrape_pti():
    ...
    for title, url in unique:
        ...
        if not is_strictly_yesterday(date, "PTI", title):
            continue
    ...
    results = deduplicate(results)
    if not results:
        debug("PTI: Kal (Yesterday) ka koi data nahi mila.")
...
def scrape_prs():
    ...
    for page_url in PRS_PAGES:
        ...
        for a in soup.find_all("a", href=True):
            ...
            low = title.lower()
            if any(x in low for x in ["home", "about", "contact", "login", "donate"]):
                continue
    ...
    results = []
    for title, url in unique:
        content, date = fetch_generic_article_content(url, "PRS India")
        if not content:
            continue

        if not is_strictly_yesterday(date, "PRS INDIA", title):
            continue
    ...
    results = deduplicate(results)
    if not results:
        debug("PRS INDIA: Kal (Yesterday) ka koi data nahi mila.")

    print(f"✅ PRS India usable: {len(results)}")
    return results
...
def scrape_india_gov():
    ...
    for page_url in INDIA_GOV_PAGES:
        print(f"🔎 India.gov.in Listing: {page_url}")
        html = fetch_url(page_url)
        if not html:
            continue
    ...
    for title, url in unique:
        ...
        if not is_strictly_yesterday(date, "INDIA.GOV.IN", title):
            continue
    ...
    results = deduplicate(results)
    if not results:
        debug("INDIA.GOV.IN: Kal (Yesterday) ka koi data nahi mila.")
...
def build_news():
    ...
    source_results["Sansad TV"] = safe_source("Sansad TV", scrape_sansad_tv)
    source_results["PTI"] = safe_source("PTI", scrape_pti)
    source_results["PRS India"] = safe_source("PRS India", scrape_prs)
